...
vector<vector<double>> Matrix_block_add(vector<vector<double>> A, int start_x, int start_y, int block_x, int block_y, vector<vector<double>>B, int k);//将A中分块矩阵更新为加B之后的值,k=-1减k=1加
vector<vector<double>> Matrix_block_replace(vector<vector<double>> A, int start_x, int start_y, int block_x, int block_y, vector<vector<double>>B, int k);//将A中分块矩阵替换为k*B
...
double Matrix_absmax(vector<vector<double>> A);                                                        //找矩阵绝对值最大的元素
double Matrix_max(vector<vector<double>> A);                                                           //找矩阵最大的元素
...
void givens(double &c, double &s, double a, double b);                                                 //计算givens变换,返回cos和sin的值
...
vector<vector<double>> givens_mul(vector<vector<double>> A, double cos, double sin, int p, int q, int k);//乘上givens变换,k=1左乘,k=-1右乘
double Non_diagonal_norm(vector<vector<double>> A);                                                  //计算A的非对角范数
vector<vector<double>> Pass_Jacobi(vector<vector<double>>A, vector<vector<double>>& J, int& k);      //过关Jacobi法求实对称矩阵特征值和特征向量，保存在A和J中
int sgn(double m);                                                                                   //符号判断函数,  >=0为1   <0为-1 
int sign(double m);                                                                                  //符号判断函数,  >0为1   <0为-1 ,0为0
int variants_num(vector<vector<double>>A, double u);                                                 //计算变号数sn(u)
double dichotomy(vector<vector<double>>A, int m, int& k);                                                    //用二分法求一个实对称三对角阵A中第m大的特征值
vector<double> Inverse_Power_method(vector<vector<double>>A, double lanta);                          //反幂法求lenta的特征向量
vector<vector<double>> two_diagonal(vector<vector<double>>A, vector<vector<double>>& U, vector<vector<double>>& V);//二对角化A=UAV.
void p_q(vector<vector<double>>B, int& p, int& q);                                                      //SVD迭代中求pq
vector<vector<double>> Wilkinson_SVD(vector<vector<double>>B, vector<vector<double>>& P, vector<vector<double>>& Q);//算法7.6.2带Wilkinson位移的SVD迭代
void cout_Singular_values(vector<vector<double>>B,int k);//输出B的奇异值,k>0升序，k=0直接输出，k<0降序
